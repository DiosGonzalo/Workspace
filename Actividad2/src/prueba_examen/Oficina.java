...
public class Oficina {
	...
	public ArrayList<Trastero> buscar(double precio){
		...
		o.forEach(trastero->{ if(trastero.getPrecio()==precio) { aux.add(trastero); }});
	...
	public ArrayList<Trastero> masCaro(){
		double caro=o.getFirst().getPrecio(); //getfirst para coger el primero de la coleccion
		for (Trastero t1 : o) {
	...
	public void ordenarPrecio() {	
	   ...
	   public List<Trastero> ordenar() {
	        ArrayList<Trastero> ordenados = new ArrayList<Trastero>(o); // Copia de la lista original //ademas puedes poner list<Trastero> nueva= o;
